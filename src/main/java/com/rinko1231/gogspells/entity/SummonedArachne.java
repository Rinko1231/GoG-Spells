...
public class SummonedArachne extends AbstractGaiaEntity implements RangedAttackMob, MagicSummon {
    ...
    public void aiStep() {
        ...
        if (this.getHealth() < this.getMaxHealth() * 0.33F) {
            this.addEffect(new MobEffectInstance(SPIDER_ASPECT.get(), -1, 0, true, false));
        }
    ...
    private void setSpawn(int id) {
        if ( id == 0) {
            ...
            if (caveSpider != null) {
                ...
                caveSpider.finalizeSpawn((ServerLevel)this.level(), this.level().getCurrentDifficultyAt(this.blockPosition()), MobSpawnType.MOB_SUMMONED, (SpawnGroupData)null, null);
