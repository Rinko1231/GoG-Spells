...
public class HolyMeleeAttackGoal extends Goal {
    ...
    protected void checkAndPerformAttack(LivingEntity target) {
        if (this.canPerformAttack(target)) {
            ...
            // 10% 几率触发圣击
            if (this.mob.getRandom().nextDouble() < this.holyStrikeChance && this.mob instanceof MagicSummon magicSummon) {
    ...
    protected void performDivineSmite(PathfinderMob mob) {
        ...
        Vec3 particleLocation = level.clip(new ClipContext(smiteLocation, smiteLocation.add((double)0.0F, (double)-2.0F, (double)0.0F), ClipContext.Block.VISUAL, ClipContext.Fluid.NONE, (Entity)null)).getLocation().add((double)0.0F, 0.1, (double)0.0F);
        ...
        List<Entity> entities = level.getEntities(this.mob, AABB.ofSize(smiteLocation, (double)(radius * 2.0F), (double)(radius * 4.0F), (double)(radius * 2.0F)));
        if(!(this.mob instanceof MagicSummon magicSummon)) return;
        SpellDamageSource damageSource =
                ((AbstractSpell) DIVINE_SMITE_SPELL.get())
                        .getDamageSource(this.mob, this.mob);
        double ATK = 8;
        ...
        for(Entity targetEntity : entities) {
            if (targetEntity.isAlive()
                ...
                EnchantmentHelper.doPostDamageEffects(this.mob, targetEntity);
            }
        }
