...
public class SummonedValkyrie extends AbstractAssistGaiaEntity implements PowerableMob, MagicSummon {
    ...
    public static AttributeSupplier.Builder createAttributes() {
        return Monster.createMonsterAttributes()
                .add(AttributeRegistry.HOLY_MAGIC_RESIST.get(), 1.25f)
                .add(AttributeRegistry.LIGHTNING_MAGIC_RESIST.get(), 1.25f)
                .add(Attributes.MAX_HEALTH, (double)160.0F).add(Attributes.FOLLOW_RANGE, (double)40.0F).add(Attributes.MOVEMENT_SPEED, 0.3).add(Attributes.ATTACK_DAMAGE, (double)12.0F).add(Attributes.ARMOR, (double)12.0F).add(Attributes.KNOCKBACK_RESISTANCE, 0.2).add((Attribute) ForgeMod.STEP_HEIGHT_ADDITION.get(), (double)1.0F);
    }
    ...
    public boolean hurt(DamageSource source, float damage) {
        ...
        if (!this.isPowered()) {
            ...
            return !this.shouldIgnoreDamage(source) && !source.isIndirect() && super.hurt(source, input);
        }
    }
    ...
    public boolean doHurtTarget(Entity entityIn) {
        if (Utils.doMeleeAttack(this, entityIn,
            ...
            if (entityIn instanceof LivingEntity) {
                ...
                if (effectTime > 0) {
                    livingEntity.addEffect(new MobEffectInstance(MobEffectRegistry.GUIDING_BOLT.get(), 20));
