...
public class SummonArachneSpell extends AbstractSpell {
    ...
    public void onCast(Level world, int spellLevel, LivingEntity entity, CastSource castSource, MagicData playerMagicData) {
        ...
        for (int i = 0; i < this.getSummonCount(spellLevel); ++i) {
            ...
            summonedArachne.addEffect(new MobEffectInstance(SUMMON_ARACHNE_TIMER.get(), summonTime, 0, false, false, false));
            world.addFreshEntity(summonedArachne);

        }


        int effectAmplifier = spellLevel - 1;
        if (entity.hasEffect(SUMMON_ARACHNE_TIMER.get())) {
            effectAmplifier += entity.getEffect(SUMMON_ARACHNE_TIMER.get()).getAmplifier() + 1;
        }

        entity.addEffect(new MobEffectInstance(SUMMON_ARACHNE_TIMER.get(), summonTime, effectAmplifier, false, false, true));
