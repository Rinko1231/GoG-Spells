...
public class SummonMummySpell extends AbstractSpell {
    ...
    public void onCast(Level world, int spellLevel, LivingEntity entity, CastSource castSource, MagicData playerMagicData) {
            ...
            for (int i = 0; i < this.getSummonCount(spellLevel); ++i) {
                ...
                summonedMummy.addEffect(new MobEffectInstance(SUMMON_MUMMY_TIMER.get(), summonTime, 0, false, false, false));
                world.addFreshEntity(summonedMummy);

            }

        int effectAmplifier = spellLevel - 1;
        if (entity.hasEffect(SUMMON_MUMMY_TIMER.get())) {
            effectAmplifier += entity.getEffect(SUMMON_MUMMY_TIMER.get()).getAmplifier() + 1;
        }

        entity.addEffect(new MobEffectInstance(SUMMON_MUMMY_TIMER.get(), summonTime, effectAmplifier, false, false, true));
